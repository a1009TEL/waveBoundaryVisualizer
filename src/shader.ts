...
const default_vertex: string = `#version 300 es
...
const default_fragment: string = `#version 300 es
...
function createShader(type: any, source: string) {
...
async function loadShader(path: string): Promise<string | null> {
    try {
...
export class Shader {
    public vertex_src: string = "";
    public fragment_src: string = "";

    public program: WebGLProgram | null = null;

    public constructor(vertex_path: string, fragment_path: string) {
    ...
    private async initShader(vertex_path: string, fragment_path: string) {
    ...
    public setUniform2f(uniform_name: string, value1: number, value2: number) {
        if (this.program) {
            this.use();
            const uniform = gl?.getUniformLocation(this.program, uniform_name);
            if (uniform != null) {
    ...
    public setUniform3f(uniform_name: string, value1: number, value2: number, value3: number) {
        if (this.program) {
            this.use();
            const uniform = gl?.getUniformLocation(this.program, uniform_name);
            if (uniform != null) {
    ...
    public setUniformMatrix4fv(uniform_name: string, value: Float32Array) {
        if (this.program) {
            this.use();
            const uniform = gl?.getUniformLocation(this.program, uniform_name);
            if (uniform != null) {
